...
from DeepReg.deepreg.util import build_dataset
import DeepReg.deepreg.model.optimizer as opt
import DeepReg.deepreg.train as train
...
def fit_deepreg(
    ...
    #print(f'dataset train: {dataset_train["moving_image"]}')
    element = dataset_train.take(1) 
    print(f'dataset train element: {element}')
    ...
    try:
        print(f'config data label: {config["dataset"]["moving_label_shape"]}')
    except:
        pass
    model: tf.keras.Model = REGISTRY.build_model(
        config=dict(
            ...
            moving_label_size=(400,3), #label size here!!!
            fixed_label_size=(400,3),
